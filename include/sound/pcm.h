 ...
 */
static inline unsigned int snd_pcm_find_min_rate(struct device *dev,
						unsigned int rates)
{
	...
	if (rates & SNDRV_PCM_RATE_KNOT)
		dev_warn(dev,
		"SNDRV_PCM_RATE_KNOT is set. max_rate may not be accurate.\n");
 ...
 * snd_pcm_rate_bit_to_rate.
 */
static inline unsigned int snd_pcm_find_max_rate(struct device *dev,
						unsigned int rates)
{
	...
	return rates ? snd_pcm_rate_bit_to_rate(1 << last_set_bit) : 0;
}
