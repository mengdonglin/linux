...
/* set the default DAI format of BE link */
	...
	struct snd_soc_tplg_link_config *cfg)
{
	...
	link->codecs = kzalloc(sizeof(struct snd_soc_dai_link_component) * link->num_codecs,
						GFP_KERNEL);
	if(!link->codecs) {
		ret = -ENOMEM;
		goto err;
...
{
	struct snd_soc_dai_link *link;
	printk("BE link id %d, %s, stream name %s, priv data size %d\n", cfg->id, cfg->name, cfg->stream_name, cfg->priv.size);

	link = snd_soc_find_dai_link(tplg->comp->card, cfg->id);
	if (!link)
		return soc_tplg_be_link_create(tplg, cfg);

	dev_dbg(tplg->dev, "Found static BE link %s, stream name %s\n", cfg->name, cfg->stream_name);
	return 0;
}
