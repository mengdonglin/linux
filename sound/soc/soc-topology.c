...
/* Find and configure an existing BE DAI.
	...
	struct snd_soc_tplg_be_dai *be)
{
	...
	tplg->pos += (sizeof(struct snd_soc_tplg_be_dai) + be->priv.size);
	return 0;
}
	...
	struct snd_soc_tplg_hdr *hdr)
{
	...
	tplg->pos += sizeof(struct snd_soc_tplg_be_dai) * count;
	...
	struct snd_soc_tplg_hdr *hdr)
{
	...
	if (manifest->size != sizeof(*manifest)) {
			...
			printk("amanda: user space manifest size %d, kernel size %d\n", manifest->size, sizeof(*manifest));
			return -EINVAL;
	}
	...
	struct snd_soc_tplg_hdr *hdr)
{
	...
	if (hdr->size != sizeof(*hdr)) {
		dev_err(tplg->dev,
			...
			hdr->type, soc_tplg_get_hdr_offset(tplg), tplg->fw->size);
		return -EINVAL;
	}
