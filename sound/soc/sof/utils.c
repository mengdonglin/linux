...
/* set up HDMI frontend dai links
 * template:
 *	.name = "Bxt HDMI Port1",
 *	.stream_name = "Hdmi1",
		  ...
		  int link_num)
{
	...
	for (i = 0; i < link_num; i++) {
		...
		snprintf(name, 32, "Bxt HDMI Port%d", i + 1);
		...
		snprintf(name, 32, "Hdmi%d", i + 1);
		links[i].stream_name = devm_kstrdup(dev, name, GFP_KERNEL);
		if (!links[i].stream_name) {
			ret = -ENOMEM;
			goto err;
		}

		snprintf(name, 32, "HDMI%d Pin", i + 1);
		...
		links[i].codec_name = devm_kstrdup(dev,
						   "snd-soc-dummy",
						   GFP_KERNEL);
		...
		links[i].codec_dai_name = devm_kstrdup(dev,
						       "snd-soc-dummy-dai",
						       GFP_KERNEL);
...
EXPORT_SYMBOL(sof_hdmi_fes_setup);
