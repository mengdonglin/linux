...
static u32 sof_hda_reg_readl(u32 __iomem *addr)
{
	return readl(addr);
}

static void sof_hda_reg_writew(u16 value, u16 __iomem *addr)
{
	writew(value, addr);
}

static u16 sof_hda_reg_readw(u16 __iomem *addr)
{
	return readw(addr);
}

static void sof_hda_reg_writeb(u8 value, u8 __iomem *addr)
{
	writeb(value, addr);
}

static u8 sof_hda_reg_readb(u8 __iomem *addr)
{
	return readb(addr);
}
...
static const struct hdac_io_ops io_ops = {
	.reg_writel = sof_hda_reg_writel,
	.reg_readl = sof_hda_reg_readl,
	.reg_writew = sof_hda_reg_writew,
	.reg_readw = sof_hda_reg_readw,
	.reg_writeb = sof_hda_reg_writeb,
	.reg_readb = sof_hda_reg_readb,
